...
public class ScoringMatrix extends ScoringScheme
{
	...
	public ScoringMatrix (String col_codes, String row_codes, int[][] scoring_matrix, int gap_penalty)
			throws InvalidScoringMatrixException
	{
		if(col_codes == null || row_codes == null || scoring_matrix == null)
			return;
		if(col_codes.length() != row_codes.length())
			throw new InvalidScoringMatrixException	("Matrix must have as many rows as columns.");

		int max_abs = 0;

		this.col_codes = col_codes + INDEL_CHAR;
		this.row_codes = row_codes + INDEL_CHAR;
		this.dimension = this.col_codes.length();

		if(col_codes.length() != row_codes.length())
			throw new InvalidScoringMatrixException("Column and Row code lengths are not equal.");

		matrix = new int[this.row_codes.length()][this.col_codes.length()];

		for(int row = 0; row < matrix.length; row++)
		{
			for(int column = 0; column < matrix[row].length; column++)
			{
				//Last column and row contains gap penalty value.
				if(column == matrix[row].length - 1 || row == matrix.length - 1)
					matrix[row][column] = gap_penalty;
				else
					matrix[row][column] = scoring_matrix[row][column];

				if (Math.abs(matrix[row][column]) > max_abs)
					max_abs = Math.abs(matrix[row][column]);
			}
		}
    ...
    /**
     * Does not update deletion scores. Just match/mismatch scores.
     */
	public void updateScores(int[][] scoring_matrix) throws InvalidScoringMatrixException
    {
        if(!(scoring_matrix.length == this.matrix.length - 1 && scoring_matrix[0].length == this.matrix[0].length-1))
            throw new InvalidScoringMatrixException("Given scoring matrix does not match size of existing scoring matrix.");

        for(int row = 0; row < scoring_matrix.length; row ++)
        {
            for(int column = 0; column < scoring_matrix[row].length; column++)
            {
                this.matrix[row][column] = scoring_matrix[row][column];
            }
        }
    }

    /**
     * Updates only delete scores. Not match/mismatch scores.
     */
    public void updateDeletionScore(int new_deletion_score)
    {
        for(int row = 0; row < this.matrix.length; row++)
        {
            for(int column = 0; column < this.matrix[row].length; column++)
            {
                ...
                this.matrix[row][column] = new_deletion_score;
            }
        }
    }
