...
public class SmithWaterman extends PairwiseAlignmentAlgorithm
{
	 ...
	 */
	protected final ObservableMatrix matrix;
	...
	public SmithWaterman(ObservableMatrix observed_matrix) { matrix = observed_matrix; }
	 ...
	 *
	 * @param s1 Input string for the first sequence
	 * @param s2 Input string for the second sequence
	 * @throws InvalidSequenceException If any of the input sequences are not valid
	 */
	protected void loadSequencesInternal (String s1, String s2)
	...
	protected void unloadSequencesInternal ()
	{
		this.seq1 = null;
		this.seq2 = null;
		this.matrix.clearInternalMatrix();
	}
	...
	protected void computeMatrix () throws IncompatibleScoringSchemeException
	{
		...
		matrix.initializeNewInternalMatrix(rows, cols);
		...
		for (r = 1; r < rows; r++)
		{
			...
			for (c = 1; c < cols; c++)
			{
				...
				ins = (Integer)matrix.get(r, c-1) + scoreInsertion(seq2.charAt(c));
				sub = (Integer)matrix.get(r-1, c-1) + scoreSubstitution(seq1.charAt(r),seq2.charAt(c));
				del = (Integer)matrix.get(r-1, c) + scoreDeletion(seq1.charAt(r));

				// choose the greatest
				matrix.set(r, c, max (ins, sub, del, 0));

				if ((Integer)matrix.get(r, c) > max_score)
				{
					// keep track of the maximum score
					max_score = (Integer)matrix.get(r, c);
		...
		IncompatibleScoringSchemeException
	{
		...
		while ((r > 0 || c > 0) && ((Integer)matrix.get(r, c) > 0))
		{
			if (c > 0)
				if ((Integer)matrix.get(r, c) == (Integer)matrix.get(r, c-1) + scoreInsertion(seq2.charAt(c)))
				{
					// insertion
			...
			if ((r > 0) && (c > 0))
			{
				...
				if ((Integer)matrix.get(r, c) == (Integer)matrix.get(r-1, c-1) + sub)
				{
					// substitution
