...
public class NeedlemanWunschSteppableAlgorithm extends neobio.alignment.NeedlemanWunsch implements SteppableAlgorithm {

    private static final int STARTING_ROW = 1, STARTING_COLUMN = 0;
    ...
    public NeedlemanWunschSteppableAlgorithm(String sequence1, String sequence2, ScoringScheme score, ObservableMatrix observed_matrix)
    ...
    public NeedlemanWunschSteppableAlgorithm(String sequence1, String sequence2, ScoringScheme score, ObservableMatrix observed_matrix, int row_to_resume_to, int column_to_resume_to)
            throws InvalidScoringMatrixException, InvalidSequenceException
    {
    ...
    public void resume()
    {
        ...
        //If the contents of the sequences change, but not the length, recompute to the same position.
    ...
    private void initializeDynamicMatrix()
    {
        ...
        try {
            for (int i = 0; i < cols_matrix; i++) {
                this.matrix.set(0, i, i * super.scoreDeletion(seq1.charAt(current_row)));
                this.matrix.set(i, 0, i * super.scoreDeletion(seq1.charAt(current_row)));
            }
    ...
    public void stepForward()
    {
        ...
        try
        {
            if(current_column == 0)//Initialize the first column.
                this.matrix.set(current_row, 0, (Integer)matrix.get(current_row -1, 0) + super.scoreDeletion(seq1.charAt(current_row)));
            else if(current_row == 0)
                this.matrix.set(0, current_column, (Integer)matrix.get(current_row -1, 0) + super.scoreDeletion(seq1.charAt(current_row)));
            else
            {
                ...
                // choose the greatest
                this.matrix.set(current_row, current_column, max (ins, sub, del));
            }

            if ((Integer)this.matrix.get(current_row, current_column) > max_score_matrix)
            {
    ...
    public boolean hasMore()
    {
        if(this.matrix == null)
            ...
            return (current_row < matrix.length()) && (current_column < matrix.length(0));
    }
